...
const getCheckInfo = async () => {
    ...
    const defaultAddress = res.result.userAddresses.find(item => item.isDefault === 0)
...
//用样式来控制切换地址
const activeAddress = ref({}) //当前活跃的地址就只有一个，是一个活跃对象
...
const createOrder = async () => {
  ...
  //跳转到支付页面成功，需要把用户的本地购物车清空，一旦提交并生成订单，后端的用户购物车就已经被清空
  cartStore.clearCart()
}
